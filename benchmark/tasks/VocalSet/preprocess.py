...
parser = argparse.ArgumentParser(
    description="Keep rest of slice less than 3 seconds(default: False)"
)
parser.add_argument(
    "--keep", help="Keep rest of slice less than 3 seconds", action="store_true"
)
parser.add_argument("--remove", help="Remove VocalSet.zip", action="store_true")
...
if __name__ == "__main__":
    ...
    for file in tqdm(archive.infolist()):
        ...
        if (
            len(file_str) == 5
            and (file_str[-2] in tech_types)
            and (file_str[-1] not in filter)
        ):
    ...
    for i in range(len(name_correction)):
        if os.path.exists(unzip_dir + name_correction[i][0]):
            os.rename(
                unzip_dir + name_correction[i][0], unzip_dir + name_correction[i][1]
            )
    ...
    data_df = pd.DataFrame(
        {"file_name": [], "tech_class": [], "singer_class": [], "file_id": []}
    )
    ...
    for cur_type in tqdm(sorted(os.listdir(unzip_dir))):
        ...
        for i, file in enumerate(files):
            chunks = partition(
                resample(remove_silence(open_file(unzip_dir + cur_type + "/" + file)))
            )
    ...
    tech_data_df = data_df[data_df["tech_class"].isin(tech_task_type)].reset_index(
        drop=True
    )
    ...
    test_type = ["f2", "f8", "m3", "m5"]
    train_val_df = tech_data_df[
        tech_data_df["singer_class"].isin(train_type)
    ].reset_index(drop=True)
    test_df = tech_data_df[tech_data_df["singer_class"].isin(test_type)].reset_index(
        drop=True
    )
    ...
    gss = GroupShuffleSplit(n_splits=1, test_size=0.125)
    train_id, valid_id = next(gss.split(train_val_df, groups=train_val_df.singer_class))
    train_df, valid_df = train_val_df.iloc[train_id].reset_index(
        drop=True
    ), train_val_df.iloc[valid_id].reset_index(drop=True)
