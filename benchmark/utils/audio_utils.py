...
def crop_audio(
    ...
    """
    assert (
        crop_to_length_in_sec is None or crop_to_length_in_sample_points is None
        ...
        if waveform.shape[-1] > crop_duration_in_sample:
            ...
            if pad:
                waveform = torch.nn.functional.pad(
                    waveform, (0, crop_duration_in_sample - waveform.shape[-1])
                )
...
def chunk_audio(
    ...
    overlap_in_sec = (
        sliding_window_size_in_sec * sliding_window_overlap_in_percent / 100
    )
    wavs = []
    for i in range(
        0,
        wav.shape[-1],
        int(sample_rate * (sliding_window_size_in_sec - overlap_in_sec)),
    ):
