...
class MLPProberBase(pl.LightningModule):
    ...
    def select_training_cfg(self, cfg, _cfg):
        ...
        cfg.lr = (
            eval(_cfg.optimizer.lr)
            if isinstance(_cfg.optimizer.lr, str)
            else _cfg.optimizer.lr
        )
    ...
    def configure_optimizers(self):
        ...
        optimizer = Optimizer(
            self.parameters(), lr=lr, weight_decay=self.hparams.l2_weight_decay
        )
    ...
    def update_metrics(self, split, y, y_pred):
        raise NotImplementedError(
            f"update_metrics() not implemented in {self.__class__.__name__}"
        )

    @torch.no_grad()
    def log_metrics(self, split):
        raise NotImplementedError(
            f"log_metrics() not implemented in {self.__class__.__name__}"
        )
...
class ProberForBertUtterCLS(MLPProberBase):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self.loss(y_pred, y)
        self.log(
            "valid_loss", loss, batch_size=y.shape[0], prog_bar=True, sync_dist=True
        )
        self.update_metrics("valid", y, y_pred)
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss = self.loss(y_pred, y)
        self.log(
            "test_loss", loss, batch_size=y.shape[0], prog_bar=True, sync_dist=True
        )
        self.update_metrics("test", y, y_pred)
    ...
    def get_loss(self):
        if self.hparams.task_type == "multilabel":
            ...
            raise NotImplementedError(
                f"get_loss() of {self.hparams.task_type} not implemented in {self.__class__.__name__}"
            )
            ...
            if self.hparams.task_type == "multilabel":
                setattr(
                    ...
                    torchmetrics.AveragePrecision(
                        task=self.hparams.task_type, num_labels=self.hparams.num_outputs
                    ),
                )
                ...
                raise NotImplementedError(
                    f"init_metrics() of {self.hparams.task_type} not implemented in {self.__class__.__name__}"
                )
    ...
    def update_metrics(self, split, y, y_pred):
        ...
        if self.hparams.task_type == "multilabel":
            ...
            raise NotImplementedError(
                f"compute_metrics() of {self.hparams.task_type} not implemented in {self.__class__.__name__}"
            )
    ...
    def log_metrics(self, split):
        ...
        if self.hparams.task_type == "multilabel":
            self.log(
                f"{split}_ap", getattr(self, f"{split}_ap").compute(), sync_dist=True
            )
            ...
            self.log(
                f"{split}_f1", getattr(self, f"{split}_f1").compute(), sync_dist=True
            )
            getattr(self, f"{split}_f1").reset()
        elif self.hparams.task_type == "multiclass":
            self.log(
                f"{split}_acc", getattr(self, f"{split}_acc").compute(), sync_dist=True
            )
            ...
            raise NotImplementedError(
                f"log_metrics() of {self.hparams.task_type} not implemented in {self.__class__.__name__}"
            )
    ...
    def log_weights(self, split):
        ...
        log_dict = {
            f"{split}_layer_{i+1}_weight": w.item() for i, w in enumerate(weights)
        }
        return log_dict
        ...
        if self.hparams.layer == "all":
            ...
            if self.hparams.normalized_weight_sum:
                self.aggregator = nn.Parameter(
                    torch.randn((1, self.hparams.n_tranformer_layer, 1))
                )
            else:
                self.aggregator = nn.Conv1d(
                    in_channels=self.hparams.n_tranformer_layer,
                    out_channels=1,
                    kernel_size=1,
                )
    ...
    def init_scatter_mean(self):
        try:
            ...
            print(
                "[Warning]: torch_scatter is not installed, use non-cuda implementation.\n"
            )
...
class ProberForBertSeqLabel(ProberForBertUtterCLS):
    ...
    def select_upstream_cfg(self, cfg, _cfg):
        ...
        pre_trained_folder = getattr(
            cfg.pretrained_extractor, "pre_trained_folder", None
        )
        huggingface_model_name = getattr(
            cfg.pretrained_extractor, "huggingface_model_name", None
        )
        ...
        cfg.processor_normalize = (
            _cfg.model.feature_extractor.pretrain.processor_normalize
        )
    ...
    def init_aggregator(self):
        if self.hparams.layer == "all":
            ...
            if self.hparams.normalized_weight_sum:
                self.aggregator = nn.Parameter(
                    torch.randn((self.hparams.n_tranformer_layer, 1, 1, 1))
                )
            else:
                self.aggregator = nn.Conv1d(
                    in_channels=self.hparams.n_tranformer_layer,
    ...
    def init_extractor(self):
        ...
        FeatureExtractor = eval(bench.NAME_TO_PRETRAIN_CLASS[model_name])
