...
class MLPProberBase(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.Adam(
            self.parameters(), lr=lr, weight_decay=self.cfg.l2_weight_decay
        )
    ...
    def get_loss(self):
        raise NotImplementedError(
            f"get_loss() not implemented in {self.__class__.__name__}"
        )

    def init_metrics(self):
        raise NotImplementedError(
            f"init_metrics() not implemented in {self.__class__.__name__}"
        )

    def compute_metrics(self, split, y, y_pred, loss):
        raise NotImplementedError(
            f"compute_metrics() not implemented in {self.__class__.__name__}"
        )
...
class ProberForBertCLS(MLPProberBase):
        ...
        if self.cfg.layer == "all":
            ...
            if self.cfg.normalized_weight_sum:
                self.aggregator = nn.Parameter(
                    torch.randn((1, self.cfg.n_tranformer_layer, 1))
                )
            else:
                self.aggregator = nn.Conv1d(
                    in_channels=self.cfg.n_tranformer_layer,
                    out_channels=1,
                    kernel_size=1,
                )
    ...
    def scatter_mean(self, y_pred, index, dim=0):
        new_y_pred = torch.zeros(
            (index.unique().shape[0], y_pred.shape[1]), device=y_pred.device
        )
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.cfg.dataset == "GS":
            with torch.no_grad():
                log_ensemble_results = gs_ensemble(
                    outputs, self.device
                )  # [(strategy_name, predicted_label, label), ...]
                best_strategy_name = log_ensemble_results.pop("best_ensemble_strategy")
                if (
                    log_ensemble_results["best_ensemble_score"]
                    > self.valid_best_ensemble_score
                ):
                    self.valid_best_ensemble_score = log_ensemble_results[
                        "best_ensemble_score"
                    ]
                    self.valid_best_ensemble_strategy = best_strategy_name

                log_ensemble_results = dict(
                    ("valid_" + k, v) for (k, v) in log_ensemble_results.items()
                )  # change the logging key name
    ...
    def test_epoch_end(self, outputs):
        ...
        with torch.no_grad():
            ...
            if self.cfg.dataset == "GS":
                log_ensemble_results = gs_ensemble(outputs, self.device)
                log_ensemble_results.pop(
                    "best_ensemble_strategy"
                )  # no need for test_best_strategy_name
                if self.valid_best_ensemble_strategy is not None:
                    log_ensemble_results["ensemble_score_val-select"] = (
                        log_ensemble_results[
                            f"ensemble_{self.valid_best_ensemble_strategy}_score"
                        ]
                    )

                log_ensemble_results = dict(
                    ("test_" + k, v) for (k, v) in log_ensemble_results.items()
                )  # change the logging key name
                log_dict.update(log_ensemble_results)
        self.log_my_dict(log_dict)
    ...
    def get_loss(self):
        if self.cfg.task_type == "multilabel":
            ...
            raise NotImplementedError(
                f"get_loss() of {self.cfg.task_type} not implemented in {self.__class__.__name__}"
            )
            ...
            if self.cfg.task_type == "multilabel":
                setattr(
                    ...
                    torchmetrics.AveragePrecision(
                        task=self.cfg.task_type, num_labels=self.cfg.num_outputs
                    ),
                )
                ...
                setattr(
                    ...
                    torchmetrics.Accuracy(
                        task=self.cfg.task_type, num_classes=self.cfg.num_outputs
                    ),
                )
                ...
                setattr(
                    self,
                    f"{split}_r2",
                    torchmetrics.R2Score(num_outputs=2, multioutput="uniform_average"),
                )
                self.all_metrics.add("r2")
                setattr(
                    self, f"{split}_arousal_r2", torchmetrics.R2Score(num_outputs=1)
                )
                self.all_metrics.add("arousal_r2")
                setattr(
                    self, f"{split}_valence_r2", torchmetrics.R2Score(num_outputs=1)
                )
                self.all_metrics.add("valence_r2")
            else:
                raise NotImplementedError(
                    f"init_metrics() of {self.cfg.task_type} not implemented in {self.__class__.__name__}"
                )
        ...
        if y is not None:
            if self.cfg.task_type != "regression":
                ...
                if self.cfg.task_type == "multilabel":
                    ...
                    raise NotImplementedError(
                        f"compute_metrics() of {self.cfg.task_type} not implemented in {self.__class__.__name__}"
                    )
...
class ProberForBertSeqLabel(MLPProberBase):
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.layer == "all":
            # use learned weights to aggregate features
            self.aggregator = nn.Parameter(
                torch.randn((self.cfg.n_tranformer_layer, 1, 1, 1))
            )
        # TODO: enable biLSTM
        # TODO: fix ddp
    ...
    def forward(self, x):
        ...
        if self.cfg.layer == "all":
            with torch.no_grad():
                x = self.model(x, layer=None, reduction="none")[
                    1:
                ]  # [12, batch_size, seq_length (249), hidden_dim]
            x = (F.softmax(self.aggregator, dim=0) * x).sum(
                dim=0
            )  # [batch_size, seq_length (249), hidden_dim]
        else:
            with torch.no_grad():
                x = self.model(
                    x, layer=int(self.cfg.layer), reduction="none"
                )  # [batch_size, seq_length (249), hidden_dim]
    ...
    def get_loss(self):
        if self.cfg.task_type == "multilabel":
            ...
            raise NotImplementedError(
                f"get_loss() of dataset {self.cfg.dataset} not implemented in {self.__class__.__name__}"
            )
        ...
        for split in ["train", "valid", "test"]:
            if self.cfg.task_type == "multilabel":
                ...
                setattr(
                    ...
                    f"{split}_binary_ap",
                    torchmetrics.AveragePrecision(
                        task="binary", average="macro", thresholds=40
                    ),
                )
                ...
                setattr(
                    self,
                    f"{split}_acc",
                    torchmetrics.Accuracy(num_classes=self.cfg.num_outputs),
                )
                self.all_metrics.add("acc")
                ...
                setattr(
                    self,
                    f"{split}_prec",
                    torchmetrics.Precision(num_classes=self.cfg.num_outputs),
                )
                self.all_metrics.add("prec")
...
class _ProberForBertUtterCLS(MLPProberBase):
    ...
    def log_metrics(self, split, on_step=False, on_epoch=True):
        ...
        if self.cfg.task_type == "multilabel":
            self.log(
                f"{split}_ap",
                getattr(self, f"{split}_ap").compute(),
                sync_dist=True,
                on_step=on_step,
                on_epoch=on_epoch,
            )
            self.log(
                f"{split}_aucroc",
                getattr(self, f"{split}_aucroc").compute(),
                sync_dist=True,
                on_step=on_step,
                on_epoch=on_epoch,
            )
            self.log(
                f"{split}_f1",
                getattr(self, f"{split}_f1").compute(),
                sync_dist=True,
                on_step=on_step,
                on_epoch=on_epoch,
            )
            ...
            self.log(
                f"{split}_acc",
                getattr(self, f"{split}_acc").compute(),
                sync_dist=True,
                on_step=on_step,
                on_epoch=on_epoch,
            )
            self.log(
                f"{split}_prec",
                getattr(self, f"{split}_prec").compute(),
                sync_dist=True,
                on_step=on_step,
                on_epoch=on_epoch,
            )
            ...
            raise NotImplementedError(
                f"log_metrics() of {self.cfg.task_type} not implemented in {self.__class__.__name__}"
            )
