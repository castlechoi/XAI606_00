...
class AudioBERTFeature(nn.Module):
    ...
    def process_wav(self, waveform):
        ...
        return self.processor(
            waveform, return_tensors="pt", sampling_rate=self.sample_rate, padding=True
        ).input_values[0]
    ...
    def sliding_window_forward(
        ...
        for i in range(0, T - window_size_in_sample + 1, stride_in_sample):
            out.append(
                self.forward(
                    input_values[:, i : i + window_size_in_sample],
                    layer=layer,
                    reduction=reduction,
                )
            )
        if allow_non_full_window and T % stride_in_sample != 0:
            out.append(
                self.forward(
                    input_values[:, -window_size_in_sample:],
                    layer=layer,
                    reduction=reduction,
                )
            )
