...
# from musichubert_hf.configuration_musichubert import MusicHubertConfig
    ...
    def __init__(self, config):
        ...
        if self.feature_extractor_cqt:
            ...
            self.feature_dimension = (
                config.conv_dim[-1] + config.feature_extractor_cqt_bins
            )
        ...
        if self.feat_proj_layer_norm:
            self.layer_norm = nn.LayerNorm(
                self.feature_dimension, eps=config.layer_norm_eps
            )
...
class MusicHubertModel(HubertModel):
    ...
    def __init__(
        ...
        self.feature_projection = MusicHubertFeatureProjection(
            config
        )  # replace Feature Projection for introcuing new feature
        ...
        if self.config.feature_extractor_cqt:
            ...
            self.feature_extractor_cqt = nnAudioFeatures.cqt.CQT(
                ...
                hop_length=self.config.sample_rate // 50,
                fmin=32.7,
                fmax=None,
                n_bins=self.config.feature_extractor_cqt_bins,
                bins_per_octave=self.config.feature_extractor_cqt_bins // 7,
                filter_scale=1,
                norm=1,
                window="hann",
                center=True,
...
class HubertEncoder_extend(HubertEncoder):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [HubertEncoderLayerExtend(config) for _ in range(config.num_hidden_layers)]
        )
